...
/* my code for key points detection*/
// Detect keypoints in image using the Harris keypoints Detectors
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &imgGray, bool bVis) {
  // Detector parameters
  ...
  t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
  std::cout << "Harris with n= " << keypoints.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << std::endl;
...
/* end of my code for key points detection*/
