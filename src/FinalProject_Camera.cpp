...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            /*
            //print out the best box Matches
            cout << "bbBestMatches: " << endl;
            std::map<int, int>::iterator itr; 
            for (itr = bbBestMatches.begin(); itr != bbBestMatches.end(); ++itr) { 
              cout << '\t' << itr->first 
                << '\t' << itr->second << '\n'; 
            } 
            */
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                  ...
                  cout << "Lidar TTC  " << "Camera TTC" <<endl;
                  cout << ttcLidar<< '\t' << ttcCamera << endl;
                    ...
                    if (bVis)
                    {
                        ...
                        cv::drawKeypoints(visImg, currBB->keypoints, visImg, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
                        char str[200];
